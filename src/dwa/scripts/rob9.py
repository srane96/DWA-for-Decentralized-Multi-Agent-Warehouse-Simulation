...
def main():
    ...
    obs = Obstacles()
    subOdom = rospy.Subscriber("/robot_8/base_pose_ground_truth", Odometry, config.assignOdomCoords)
    subLaser = rospy.Subscriber("/robot_8/base_scan", LaserScan, obs.assignObs, config)
    ...
    pub = rospy.Publisher("/robot_8/cmd_vel", Twist, queue_size=1)
    speed = Twist()
    ...
    while not rospy.is_shutdown():
        if (atGoal(config,x) == False):
            ...
            if config.stall_count >= 100:
                goalComplete = config.goalCompleteRequest('r9',config.goal_name,0.0, False)
            ...
            if config.canSendCompletionRequest:
                goalComplete = config.goalCompleteRequest('r9',config.goal_name,config.job_end - config.job_start, True)
                config.canSendCompletionRequest = False
        ...
        if not config.busy:
            pub.publish(speed)
            #time.sleep(5)
            goalCoord = config.goalServiceRequeset('r9')
