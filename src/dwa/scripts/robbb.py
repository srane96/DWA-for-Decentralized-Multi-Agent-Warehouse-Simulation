...
from warehouse_manager.srv import Robot_Task_Request
...
class Config():
    ...
    def goalServiceRequeset(self, name):
        ...
        name = "Hello"
        try:
            goalCoord = rospy.ServiceProxy('/report_task_complete',Robot_Task_Request)
    ...
    while not rospy.is_shutdown():
        if (atGoal(config,x) == False):
            ...
            if config.canSendCompletionRequest:
                final_dist = config.goalX - config.start_x + config.goalY - config.start_y
