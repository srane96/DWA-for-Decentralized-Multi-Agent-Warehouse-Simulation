...
def main():
    ...
    obs = Obstacles()
    subOdom = rospy.Subscriber("/robot_2/base_pose_ground_truth", Odometry, config.assignOdomCoords)
    ...
    while not rospy.is_shutdown():
        if (atGoal(config,x) == False):
            ...
            if config.stall_count >= 100:
                goalComplete = config.goalCompleteRequest('r3',config.goal_name,0.0, False)
            ...
            if config.canSendCompletionRequest:
                goalComplete = config.goalCompleteRequest('r3',config.goal_name,config.job_end - config.job_start, True)
                config.canSendCompletionRequest = False
