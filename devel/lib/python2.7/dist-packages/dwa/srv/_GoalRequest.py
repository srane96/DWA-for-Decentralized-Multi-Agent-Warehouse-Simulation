...
class GoalRequestResponse(genpy.Message):
  _md5sum = "0fef0002ac8ab89f12f4a92a7037d505"
...
bool success
"""
  __slots__ = ['goal_x','goal_y','success']
  ...
  def serialize(self, buff):
    ...
    try:
      _x = self
      buff.write(_get_struct_2qB().pack(_x.goal_x, _x.goal_y, _x.success))
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      (_x.goal_x, _x.goal_y, _x.success,) = _get_struct_2qB().unpack(str[start:end])
...
class GoalRequest(object):
  ...
  _md5sum = 'f89d6f624961c5b5db599ef712099bcc'
